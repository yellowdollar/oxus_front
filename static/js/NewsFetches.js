    ...
    .then(data => {
        ...
        data.forEach(each => {
            ...
            if (photoPaths[0] === "No Photo") {
                ...
                news_image.src = `http://147.45.233.158/api/news/upload?photo_path=${photoPaths[0]}`;
            }
            ...
            news_title.innerText = each.title;
            ...
            block_item.addEventListener('click', function() {
                ...
                opened_img.src = `http://147.45.233.158/api/news/upload?photo_path=${photoPaths[0]}`;
                ...
                for (let i = 0; i < photoPaths.length; i++) {
                    ...
                    cl_image.src = `http://147.45.233.158/api/news/upload?photo_path=${photoPaths[i]}`;
                    image_div.appendChild(cl_image);
                    closed_images.appendChild(image_div);

                    image_div.addEventListener('click', function() {
                        opened_img.src = `http://147.45.233.158/api/news/upload?photo_path=${photoPaths[i]}`
                    });
                }
