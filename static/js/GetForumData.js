document.addEventListener('DOMContentLoaded', function() {
    ...
    if(forumId) {
        // MAIN FETCH
        fetch(`https://jewelryforum.tj/api/forum/get_forum?id=${forumId}`, {
            method: 'GET'
        })
        .then(response => {
            if(!response.ok) {
        ...
        .then(data => {
            ...
            // GET SPEAKERS FETCH
            fetch(`https://jewelryforum.tj/api/speakers/get_speakers?id=${data[0].id}`, {
                method: 'GET'
            })
            .then(response => {
                if(!response.ok) {
            ...
            .then(data => {
                ...
                data.forEach(each => {
                    ...
                    img.src = `https://jewelryforum.tj/api/news/upload?photo_path=${each.photo_path}`;
            ...
            // GET HONORARY FETCH
            fetch(`https://jewelryforum.tj/api/honorary/get_honorary?forum_id=${data[0].id}`, {
                method: 'GET'
            })
            .then(response => {
                if(!response.ok) {
